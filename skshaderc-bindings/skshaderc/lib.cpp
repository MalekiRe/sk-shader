...
void* compile_file_2(const char *filename, const char *contents, compiler_settings_t *settings, size_t *sks_size) {
    skg_shader_file_t file;
    if (sksc_compile(filename, contents, &settings->shaderc, &file)) {

    			// Turn the shader data into a binary file
    			void  *sks_data;
    			size_t sks_size2;
    			sksc_build_file(&file, &sks_data, &sks_size2);
    			*sks_size = sks_size2;
                return sks_data;
    }
    return (void*)-1;
